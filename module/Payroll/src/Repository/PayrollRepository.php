...
class PayrollRepository extends HrisRepository {
    ...
    public function getDaysFromJoinDate($employeeId, $sheetNo) {
        ...
        $boundedParameter['employeeId'] = $employeeId;
        $boundedParameter['sheetNo'] = $sheetNo;
        $sql = "SELECT to_date('2022-09-30','YYYY-mm-dd') - TRUNC(JOIN_DATE) AS DAYS_FROM_JOIN_DATE
                FROM HRIS_SALARY_SHEET_EMP_DETAIL WHERE EMPLOYEE_ID = :employeeId AND SHEET_NO = :sheetNo";
        $resultList = $this->rawQuery($sql, $boundedParameter);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('Result not found.');
...
FROM HRIS_MONTH_CODE  where MONTH_ID=(:monthId-1)";
	...
	public function getSsfemployee($employeeId){
        $boundedParameter = [];
        $boundedParameter['employeeId'] = $employeeId;
        $sql = "SELECT 
				CASE 
				...
				'1'
				END 
				AS SSF_EMPLOYEE_TYPE FROM HRIS_EMPLOYEES WHERE EMPLOYEE_ID =:employeeId";
        $resultList = $this->rawQuery($sql, $boundedParameter);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['SSF_EMPLOYEE_TYPE'];
    }
	
	 public function getManagerAccesibleIncome($monthId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
        $sql = "SELECT nvl(SUM(val),0) AS MANAGER_TAXABLE_INCOME FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID IN (SELECT EMPLOYEE_ID FROM HRIS_EMPLOYEES
				WHERE
				DESIGNATION_ID = 51) AND pay_id = (SELECT PAY_ID FROM HRIS_PAY_SETUP WHERE PAY_CODE ='TAI')
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID =:monthId and salary_type_id = 1)";
        $resultList = $this->rawQuery($sql, $boundedParameter);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['MANAGER_TAXABLE_INCOME'];
    }
	
	public function getManagerRole($employeeId){
        $boundedParameter = [];
        $boundedParameter['employeeId'] = $employeeId;
        $sql = "SELECT 
				CASE 
				WHEN
				DESIGNATION_ID = (SELECT DESIGNATION_ID FROM HRIS_DESIGNATIONS WHERE DESIGNATION_CODE ='MD')
				THEN
				'0'
				ELSE
				'1'
        ...
        return $resultList[0]['MANAGER_ROLE'];
    }
	
	
	public function getManagerTax($monthId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
        $sql = "SELECT SUM(val) AS MANAGER_TAX FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID IN (SELECT EMPLOYEE_ID FROM HRIS_EMPLOYEES
				WHERE
				DESIGNATION_ID = 51) AND pay_id = (SELECT PAY_ID FROM HRIS_PAY_SETUP WHERE PAY_CODE ='TM')
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID not in (:monthId) and start_date between '17-JULY-22' and '16-JULY-23' )";
        $resultList = $this->rawQuery($sql, $boundedParameter);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['MANAGER_TAX'];
    }
	
		public function getBonusTaxable($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;

        $sql = "SELECT nvl(val,0) AS BONUS_TAXABLE FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID = $employeeId AND pay_id = 14
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID=$monthId-1)
				";
        $resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['BONUS_TAXABLE'];
    }
	
		public function getBonusMonthTax($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;

        $sql = "SELECT nvl(val,0) AS BONUS_MONTH_TAX FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID = $employeeId AND pay_id = 26
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID=$monthId-1)
				";
        $resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
        ...
        return $resultList[0]['BONUS_MONTH_TAX'];
    }
	
		public function getBonusPreTax($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;

        $sql = "SELECT nvl(val,0) AS BONUS_PREV_MTH FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID = $employeeId AND pay_id = 27
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID=$monthId-2)
				";
        $resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['BONUS_PREV_MTH'];
    }
	
		public function getPreviousContribution($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;

        $sql = "SELECT nvl(val,0) AS BONUS_PREV_MTH FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID = $employeeId AND pay_id = 20
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID=$monthId-1)
				";
        $resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
	...
	public function getPreviousMonthBonus($monthId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		//$boundedParameter['employeeId'] = $employeeId;
		
		$sql = "SELECT nvl(SUM(val),0) AS PREVIOUS_MONTH_BONUS FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID IN (SELECT EMPLOYEE_ID FROM HRIS_EMPLOYEES
				WHERE
				DESIGNATION_ID = 51) AND pay_id = 47
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE  SALARY_TYPE_ID = 2 
				AND MONTH_ID <= $monthId and start_date between '17-JULY-22' and '16-JULY-23' )";
		
		$resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['PREVIOUS_MONTH_BONUS'];
    }
	
	public function getPreviousManagerTa($monthId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		
		$sql = "SELECT SUM(val) AS PREVIOUS_MANAGER_TA FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID IN (SELECT EMPLOYEE_ID FROM HRIS_EMPLOYEES
				WHERE
				DESIGNATION_ID = 51) AND pay_id = (SELECT PAY_ID FROM HRIS_PAY_SETUP WHERE PAY_CODE ='TM')
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE MONTH_ID <= $monthId and start_date between '17-JULY-22' and '16-JULY-23'  and salary_type_id = 2)";
		
		$resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['PREVIOUS_MANAGER_TA'];
    }
	
		public function getLeaveMonth($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;
		
		$sql = "select ((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30 - mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1)
				as LEAVE_MONTH
				from hris_month_code mc where fiscal_year_id = (select fiscal_year_id from 
				hris_month_code where MONTH_ID =$monthId)  and mc.fiscal_year_month_no = 1";
		
		$resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['LEAVE_MONTH'];
    }
	
	public function getLeaveExtraMonth($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;
		...
		$sql = "select 
				CASE
				WHEN
				mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1) = 0
				THEN 0
				WHEN
				mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1) < .2
				THEN
				.2
				WHEN
				mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1) < .4
				THEN
				.4
				WHEN
				mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1) <= .6
				THEN
				.6
				WHEN
				mod(((select event_date from hris_job_history where service_event_type_id = 21 and employee_id = $employeeId ) - mc.from_date)/30,1) <= .8
				THEN
				.8
				ELSE
				1 END
				as LEAVE_EXTRA_MONTH
				from hris_month_code mc where fiscal_year_id = (select fiscal_year_id from 
				hris_month_code where MONTH_ID =$monthId)  and mc.fiscal_year_month_no = (select fiscal_year_month_no from 
        ...
        return $resultList[0]['LEAVE_EXTRA_MONTH'];
    }
	
	public function getLeaveTaken($employeeId){
        $boundedParameter = [];
		$boundedParameter['employeeId'] = $employeeId;
		
		$sql = "select sum(no_of_days) as LEAVE_TAKEN from hris_employee_leave_request 
			where employee_id = $employeeId and status = 'AP' and leave_id in (36,35) 
			and start_date between (select start_date from hris_fiscal_years where fiscal_year_id = 8)
			and (select end_date from hris_fiscal_years where fiscal_year_id =8)";
		
		$resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['LEAVE_TAKEN'];
    }
	
	public function getPreviousLeave($employeeId){
        $boundedParameter = [];
		$boundedParameter['employeeId'] = $employeeId;
		
		$sql = "select nvl(sum(previous_year_bal),0) as PREV_BAL from hris_employee_leave_assign
					where leave_id in (35,36) and employee_id = $employeeId";
		
		$resultList = $this->rawQuery($sql);
        if (!(sizeof($resultList) == 1)) {
            throw new Exception('No Report Found.');
        }
        return $resultList[0]['PREV_BAL'];
    }
	
	
	public function getPreviousBonusTax($monthId,$employeeId){
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
		$boundedParameter['employeeId'] = $employeeId;
		
		$sql = "SELECT nvl(SUM(val),0) AS PREV_BNS_TAX FROM HRIS_SALARY_SHEET_DETAIL
				WHERE EMPLOYEE_ID = $employeeId AND pay_id = 27
				AND SHEET_NO IN (SELECT SHEET_NO FROM HRIS_SALARY_SHEET WHERE  SALARY_TYPE_ID = 2 
				AND MONTH_ID <= $monthId and start_date between '17-JULY-22' and '16-JULY-23' )";
        ...
        return $resultList[0]['PREV_BNS_TAX'];
    }
}
